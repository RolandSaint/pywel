...
The answer implementation follows applicable incoming `quest.organization` edges when the question asks for an organization's quests. It preserves patch, platform, spoiler, retraction, supersession, bounded output and evidence rules. A leadership request cannot use a role/contact claim as proof. No API operation, response schema, compact tuple, default, or codebook layout changes.
