...
A temporary read-only collector inspected ordinary public CrimsonDB HTML using an identifying user agent, the site's robots rules, one request per second, same-host redirects and bounded response sizes. It stopped rather than bypassing access/rate restrictions. The completed pass used 212 requests and produced a selected-field projection, not retained page bodies, scripts, media or recovered game files. Source pass: [34628099692](https://github.com/RolandSaint/pywel/actions/runs/34628099692), artifact 10275232695. The projection hash and complete admitted/excluded identity ledger are in [s01-equipment-review.json](../quality/s01-equipment-review.json). The artifact is a temporary audit aid, not a required runtime dependency.
