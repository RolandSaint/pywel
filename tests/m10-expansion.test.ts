...
describe("M10 published-expansion regression (not a new release acceptance)", () => {
  it("preserves the approved R01 content boundary and original publication scope", async () => {
    ...
    const published = { ...additions, scope_id: "pywel-post-release-m7-r01", evidence_cutoff: "2026-09-10",
      canonical_files: additions.canonical_files.filter((file: { path: string }) => !file.path.endsWith("/g02-official-catchup.json") && !file.path.endsWith("/s01-equipment.json") && !file.path.endsWith("/s02-acquisition.json") && !file.path.endsWith("/s03-recipe-inputs.json") && !file.path.endsWith("/s04-recipe-outputs.json") && !file.path.endsWith("/s05-quest-rewards.json")),
      record_ids: Object.fromEntries(Object.entries(additions.record_ids as Record<string, string[]>).map(([key, ids]) => [key, ids.filter(id => !id.startsWith("pat_g02") && !id.startsWith("evd_g02") && !id.startsWith("rcp_g02") && !id.includes("_s01") && !id.includes("_s02") && !id.includes("_s03") && !id.includes("_s04") && !id.includes("_s05"))])),
